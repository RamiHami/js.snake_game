...
function prepareGameField() {
    // Создаем счёт
    var getScoreRow = document.getElementById("score");
    var scoreRow = document.createElement("span");
    getScoreRow.append(scoreRow);
    scoreRow.id = "scoreSpan";
    scoreRow.innerHTML = "Score: " + score;
...
function haveFood(unit) {
    ...
    if (unit_classes.includes('food-unit')) {
        check = true;
        createFood();
        var getScore = document.getElementById("scoreSpan");
        score++;
...
function delWall() {
    var getWall = document.getElementsByClassName('wall-unit')[0];
    var array = getWall.getAttribute('class').split(' ');
    if (array.includes('wall-unit')) {
        getWall.setAttribute('class', array[0] + " " + array[1]);
    }
}
...
function createWall() {
    ...
    while (!wallCreated) { //пока еду не создали
        // рандом
        ...
        console.log(wall_cell_classes + " asd ")
        // проверка на змейку
